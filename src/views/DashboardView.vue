...
const monthlyOptions = computed(() => ({
  ...
  plotOptions: {
    spline: {
      lineWidth: 2,
      marker: { enabled: monthly.value.labels.length <= 15, radius: 3 },
    },
  },
  credits: { enabled: false },
}));
...
const weeklyTotal = computed(() =>
  weeklyBreakdown.value.reduce((s, b) => s + b.count, 0),
);
...
const monthlyBreakdownTotal = computed(() =>
  monthlyBreakdown.value.reduce((s, b) => s + b.count, 0),
);
...
const allTimeBreakdown = computed(() => {
  return allSubjects.value
    .map((name) => ({
      name,
      count: store.getClassCountTotal(name),
...
const monthlyPieOptions = computed(() => {
  ...
  return {
    ...
    title: { text: undefined },
    tooltip: {
      pointFormat: "<b>{point.y}</b> classes ({point.percentage:.0f}%)",
      borderRadius: 8,
      borderWidth: 0,
      style: { fontSize: "12px" },
    },
...
const monthStats = computed(() => {
  ...
  for (const rec of store.dayRecords) {
    ...
    if (
      ry === y &&
      rm === m + 1 &&
      (rec.classes.length > 0 || rec.tags.length > 0)
    ) {
...
<template>
  <div class="dash">
    ...
    <div class="rx-card chart-card">
      ...
      <div v-if="weekly.subjects.length === 0" class="dash-empty">
        No data yet for the past 7 days.
      </div>
    ...
    <div class="rx-card chart-card">
      ...
      <div v-if="monthly.subjects.length === 0" class="dash-empty">
        No data yet this month.
      </div>
      <template v-else>
        <Chart :key="'m-' + chartKey" :options="monthlyOptions" />
        <p class="month-summary">
          <strong>{{ monthStats.classes }}</strong> classes across
          <strong>{{ monthStats.days }}</strong> days this month.
        </p>
      </template>
    </div>
        ...
        >
          ...
          <div class="breakdown-bar-wrap">
            <div
              ...
              :style="{
                width:
                  monthlyBreakdownTotal > 0
                    ? (item.count / monthlyBreakdownTotal) * 100 + '%'
                    : '0%',
                background: item.color,
              }"
            />
    ...
    <div class="rx-card">
      ...
      <div v-else class="breakdown-list">
        <div
          v-for="item in allTimeBreakdown"
          :key="item.name"
          class="breakdown-row"
        >
          <span class="breakdown-name">
          ...
          <div class="breakdown-bar-wrap">
            <div
              ...
              :style="{
                width:
                  store.totalClassesAttended > 0
                    ? (item.count / store.totalClassesAttended) * 100 + '%'
                    : '0%',
                background: item.color,
              }"
            />
